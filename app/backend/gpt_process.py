...
class ApiCaller:
    def __init__(self, api_key):
        ...
        self.llm_api_connector_url = (
            current_app.config["T2P_LLM_API_CONNECTOR_URL"] + "/call_openai"
        )
        # Do not log secrets; only log the target URL
        logger.debug(
            "ApiCaller initialized", extra={"connector_url": self.llm_api_connector_url}
        )
    ...
    def call_api(self, system_prompt, user_text):
        ...
        try:
            ...
            logger.info(
                "Calling LLM API connector", extra={"url": self.llm_api_connector_url}
            )
            ...
            if response.status_code != 200:
                ...
                raise RuntimeError(
                    f"LLM API connector returned status {response.status_code}: {response.text}"
                )
            ...
            if not msg:
                logger.error(
                    "LLM API response missing 'message' field",
                    extra={"response_keys": list(response_data.keys())},
                )
                raise ValueError("LLM API response does not contain 'message' field")

            logger.debug(
                "LLM API message received",
                extra={
                    "message_type": type(msg).__name__,
                    "message_length": len(msg) if isinstance(msg, str) else None,
                },
            )
            return msg
            ...
            raise RuntimeError(
                f"Failed to connect to LLM API connector: {str(e)}"
            ) from e
        except json.JSONDecodeError as e:
            logger.exception("Failed to parse JSON response from LLM API connector")
            raise ValueError(
                f"LLM API connector returned invalid JSON: {str(e)}"
            ) from e
    ...
    def conversion_pipeline(self, process_description):
        logger.info(
            ...
            extra={
                "description_length": (
                    len(process_description)
                    if isinstance(process_description, str)
                    else None
                )
            },
        )
        ...
        try:
            ...
            logger.debug(
                ...
                extra={
                    "json_length": (
                        len(json_data) if isinstance(json_data, str) else None
                    )
                },
            )
            ...
            if isinstance(json_data, str):
                ...
                if json_data.startswith("```"):
                    logger.debug(
                        "Removing markdown code block wrapper from LLM response"
                    )
                    ...
                    json_data = json_data.strip()
                    logger.debug(
                        "Cleaned JSON", extra={"cleaned_preview": json_data[:200]}
                    )
            ...
            try:
                ...
                logger.error(
                    "Failed to parse BPMN JSON from LLM",
                    extra={"json_preview": json_data[:500], "error": str(e)},
                )
                raise ValueError(
                    f"LLM returned invalid JSON: {str(e)}. Response preview: {json_data[:200]}"
                ) from e
    ...
    def generate_bpmn_json(self, user_description):
        ...
        logger.debug(
            "Generating BPMN JSON prompt",
            extra={
                "description_length": (
                    len(user_description) if isinstance(user_description, str) else None
                )
            },
        )
        system_prompt = (
        ...
        logger.debug(
            "Received BPMN JSON from LLM API",
            extra={
                "json_length": (
                    len(json_output) if isinstance(json_output, str) else None
                )
            },
        )
